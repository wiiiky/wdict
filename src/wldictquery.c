...
GType wl_dict_lang_get_type(void)
{
    ...
    if(G_UNLIKELY(etype==0)) {
        ...
        etype=g_enum_register_static(g_intern_static_string("WlDictLang"),
                                     values);
    }
    return etype;
}
    ...
    for(i=0; i<len; i++) {
        ...
        if(td->cb)
            td->cb(wl_dict_lang_get_enum (from),
                   wl_dict_lang_get_enum (to),
                   src,dst,td->cbData,NULL);
    }
...
{
    g_return_val_if_fail(WL_IS_DICT_LANG(from) && WL_IS_DICT_LANG(to),NULL);
                         ...
                         WlDictQueryCallback cb,gpointer userData)
{
    ...
    GTask *task=g_task_new(query,query->cancel,onTaskReadyCallback,NULL);
